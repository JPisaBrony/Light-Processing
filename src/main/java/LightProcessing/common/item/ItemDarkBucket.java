...
public class ItemDarkBucket extends Item
{
    /** field for checking if the bucket has been filled. */
    private int isFull;

    public ItemDarkBucket(int full)
    {
        super();
        this.maxStackSize = 1;
        this.isFull = full;
            ...
            if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
            {
                ...
                if (this.isFull == 0)
                {
                    ...
                    if (par2World.getBlockMaterial(i, j, k) == Material.water && par2World.getBlockMetadata(i, j, k) == 0)
                    {
                        ...
                        if (--par1ItemStack.stackSize <= 0)
                        {
                            return new ItemStack(LPItems.ItemDarkBucketFull);
                        }

                        if (!par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(LPItems.ItemDarkBucketFull)))
                        {
                            par3EntityPlayer.dropPlayerItem(new ItemStack(LPItems.ItemDarkBucketFull.itemID, 1, 0));
                        }
                ...
                else
                {
                    if (this.isFull < 0)
                    {
                        return new ItemStack(LPItems.ItemDarkBucketEmpty);
                    }
