...
public class BlockLightBlock extends Block {
	...
	public boolean shouldSideBeRendered(IBlockAccess par1IBlockAccess,
		...
		if (par5 == 0
				&& par1IBlockAccess.getBlockId(par2, par3, par4) != IDRef.LIGHT_BLOCK_ID) {
			return true;
		}
		return false;
	}
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3,
		...
		if (par5Entity instanceof EntityLiving) {
			((EntityLiving) par5Entity).addPotionEffect(new PotionEffect(
					Potion.nightVision.getId(), 400, 50));
		}

		// cures zombie villagers

		if (par5Entity instanceof EntityZombie
				&& ((EntityZombie) par5Entity).isVillager()) {
			...
			entityvillager.addPotionEffect(new PotionEffect(
					Potion.confusion.id, 200, 0));
			par5Entity.worldObj.playAuxSFXAtEntity((EntityPlayer) null, 1017,
					(int) par5Entity.posX, (int) par5Entity.posY,
					(int) par5Entity.posZ, 0);
		}
		...
		if (Methods.isPowered(par1World, par2, par3, par4)) {
			...
			boolean xpos = (par1World.doesBlockHaveSolidTopSurface(par2 + 1,
					par3, par4));
			boolean xneg = (par1World.doesBlockHaveSolidTopSurface(par2 - 1,
					par3, par4));
			boolean zpos = (par1World.doesBlockHaveSolidTopSurface(par2, par3,
					par4 + 1));
			boolean zneg = (par1World.doesBlockHaveSolidTopSurface(par2, par3,
					par4 - 1));
			double speed = 0.2;
			double cSpeed = 0.25;
			...
			if (xpos && zpos && zneg && !xneg) {
				...
				if (Math.abs(par5Entity.motionX) > Math.abs(par5Entity.motionZ)
					...
					par5Entity.motionZ = -speed;
				} else if (Math.abs(par5Entity.motionZ) > Math
						.abs(par5Entity.motionX) && par5Entity.motionZ > 0) {
					par5Entity.motionX = -speed;
			...
			else if (Math.abs(par5Entity.motionX) > Math
					.abs(par5Entity.motionZ) && par5Entity.motionX > 0) {
				par5Entity.motionX = par5Entity.motionX + speed;
			} else if (Math.abs(par5Entity.motionX) > Math
					.abs(par5Entity.motionZ) && par5Entity.motionX < 0) {
				par5Entity.motionX = par5Entity.motionX - speed;
			} else if (Math.abs(par5Entity.motionX) < Math
					.abs(par5Entity.motionZ) && par5Entity.motionZ > 0) {
