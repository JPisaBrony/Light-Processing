...
public class LightTreeGen extends WorldGenerator {
	...
	public LightTreeGen(boolean par1) {
		this(par1, 4, 0, 0, false);
	}

	public LightTreeGen(boolean par1, int par2, int par3, int par4, boolean par5) {
		super(par1);
				...
				for (int l1 = par3 - b0; l1 <= par3 + b0 && flag; ++l1) {
					for (j1 = par5 - b0; j1 <= par5 + b0 && flag; ++j1) {
						if (i1 >= 0 && i1 < 256) {
							...
							if (!isAir && !block.isLeaves(par1World, l1, i1, j1) && k1 != Block.grass.blockID && // What blocks the tree will generate on
							k1 != Block.dirt.blockID && !block.isWood(par1World, l1, i1, j1))
				...
				if (isSoil && par4 < 256 - l - 1) {
							...
							for (int l2 = par5 - i2; l2 <= par5 + i2; ++l2) {
								...
								if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || par2Random.nextInt(2) != 0 && k1 != 0) {
									...
									if (block == null || block.canBeReplacedByLeaves(par1World, j2, j1, l2)) {
										this.setBlockAndMetadata(par1World, j2, j1, l2, IDRef.DARK_LEAF_ID, this.metaLeaves);
									}
								}
							}
						}
					...
					for (j1 = 0; j1 < l; ++j1) {
						...
						if (k1 == 0 || block == null || block.isLeaves(par1World, par3, par4 + j1, par5)) {
							this.setBlockAndMetadata(par1World, par3, par4 + j1, par5, IDRef.LIGHT_WOOD_ID, this.metaWood);
						}
					}
					return true;
				}
