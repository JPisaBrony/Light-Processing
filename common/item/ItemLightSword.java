...
public class ItemLightSword extends ItemSword{
	...
	public ItemLightSword(int par1, EnumToolMaterial par2EnumToolMaterial) {
		super(par1, par2EnumToolMaterial);
		this.setUnlocalizedName("LightSword");
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer player) {
		if(world.getWorldTime() < 12500) {
			if(Methods.lightCheck(world, (int)player.posX, (int)player.posY, (int)player.posZ, 7)) {
				if(Methods.hasItem(player, IDRef.LIGHT_BALL_ID)) {
					...
					*/
					...
					for(i = 0; i < entityList.size(); i++) {
						...
						if(!ent.canBeCollidedWith())
							continue;
						ent.attackEntityFrom(DamageSource.outOfWorld, 20F);
						useItem++;
						if(useItem >= 5)
							break;
					}
