...
public class ItemDarkBucket extends Item
{
    ...
    public ItemDarkBucket(int par1, int par2)
    {
        super(par1);
        this.maxStackSize = 1;
        this.isFull = par2;
            ...
            if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
            {
                ...
                if (this.isFull == 0)
                {
                    ...
                    if (par2World.getBlockMaterial(i, j, k) == Material.water && par2World.getBlockMetadata(i, j, k) == 0)
                    {
                        ...
                        if (--par1ItemStack.stackSize <= 0)
                        {
                            return new ItemStack(Items.ItemDarkBucketFull);
                        }

                        if (!par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(Items.ItemDarkBucketFull)))
                        {
                            par3EntityPlayer.dropPlayerItem(new ItemStack(Items.ItemDarkBucketFull.itemID, 1, 0));
                        }
